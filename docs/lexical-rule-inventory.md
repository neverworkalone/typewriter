...
PR #123 post-merge correction 이후 canonical snapshot은 1,320 records / 1,278 starts / 42 reference-only / 1,579 senses다. [`data/validation/canonical-semantic-decision-source.json`](../data/validation/canonical-semantic-decision-source.json)이 authored decision source이며, current semantic coverage/review/audit projection은 이 source와 canonical content에서 validation 때마다 memory에서 재구성된다. Review pass는 모든 record/sense를 포함하고 open blocker 0건을 요구한다. 기계적으로 확정 가능한 duplicate/nested/usage-variant/overlapping pair는 authored `distinct`/`retain`으로 덮을 수 없고, 변경된 record는 [`canonical-semantic-correction-manifest.json`](../data/validation/canonical-semantic-correction-manifest.json)의 별도 decision evidence와 correction history를 요구한다.
