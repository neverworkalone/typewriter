...
PR #120 이후 canonical snapshot은 1,320 records / 1,278 starts / 42 reference-only / 1,607 senses다. [`data/validation/canonical-semantic-review.json`](../data/validation/canonical-semantic-review.json), [`canonical-semantic-coverage.json`](../data/validation/canonical-semantic-coverage.json), [`canonical-semantic-audit.json`](../data/validation/canonical-semantic-audit.json)이 같은 canonical content digest에 결속되어 있으며, review pass는 모든 record/sense를 포함하고 open blocker 0건을 요구한다.
