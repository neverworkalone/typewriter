...
| M6 baseline metrics and quality gates | Versioned canonical snapshot, reproducible metrics, search evidence, and predeclared quality-gate contract | `docs/m6-1-quality-baseline.md` and its machine-readable JSON companion | Keep the issue-start canonical digest and gate version fixed; later canonical snapshots use a new version. |
