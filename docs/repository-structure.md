...
| M6 baseline metrics and quality gates | Versioned canonical snapshot, reproducible metrics, generated report, shared exact-result candidate projection, search evidence, and predeclared quality-gate contract | `docs/m6-1-quality-baseline.md`, `docs/m6-1-quality-baseline.json`, `scripts/validate/m6-1-quality-baseline.template.md`, and `src/domain/exact-search-candidates.js` | Keep the issue-start canonical digest and gate version fixed; later canonical snapshots use a new version. |
| M6-4 writer-facing quality audit | Reproducible M6-1 sample selection, tamper rejection, and issue report | `docs/m6-4-quality-audit-sample.json`, `docs/m6-4-quality-audit-report.md`, `scripts/validate/m6-4-quality-audit.mjs`, and `tests/m6-4-quality-audit.test.mjs` | Normal CI checks the committed sample. Rebuild while issue-start inputs match; verify the immutable historical snapshot after later canonical or runtime changes. Preserve the M6-1 seed, sample rules, canonical digest, and `HOLD` while human evidence is missing. |
