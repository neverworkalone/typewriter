...
50-row result is imported and the canonical snapshot is 628 starts. The corrected
second corrected chronological timing run measures 594.282 seconds across 56
processed starts (10.612179 seconds per processed start) and passes the fixed
gate. The editorial session begins before the first timing pass; editorial and
audit decision artifacts record `finalized_at` after the last timing stop and
are bound to the completion inputs by digest. The stage only sets
...
Wave B remains separately unauthorized. Run
