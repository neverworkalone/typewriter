...
test('builds a read-only SQLite dictionary with representative lookups', async () => {
  ...
  try {
    ...
    try {
      assert.equal(countRows(database, 'records'), 470);
      assert.equal(countRows(database, 'search_forms'), 546);
      assert.equal(countRows(database, 'senses'), 557);
      assert.equal(countRows(database, 'relations'), 449);
    ...
    try {
      assert.equal(countRows(reopened, 'records'), 470);
...
test('rebuilds from an empty output and protects canonical input', async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(countRows(freshDatabase, 'records'), 470);
    } finally {
      freshDatabase.close();
    }
