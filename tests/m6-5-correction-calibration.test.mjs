...
test('M6-5 verifier rejects status, count, source, case, workload, gate, and unknown-field tampering', async () => {
  ...
  const mutations = [
    ...
    ['per-type count', (value) => { value.disposition_counts_by_type.relation_gap = 79; }],
    ['workload result', (value) => { value.workload.writer_task_results_recorded = 100; }],
    ['workload status', (value) => { value.workload.status = 'PASS'; }],
    ['ranking minimum', (value) => { value.workload.ranking_writer_choices_required_for_change = 19; }],
