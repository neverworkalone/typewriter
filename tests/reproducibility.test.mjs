...
test('two independent builds have the same logical snapshot and provenance', async () => {
  ...
  try {
    ...
    assert.equal(first.metadata.record_count, '432');
    assert.equal(first.metadata.start_count, '390');
    assert.equal(first.metadata.reference_only_count, '42');
    assert.equal(first.metadata.candidate_count, '390');
    assert.equal(first.metadata.search_form_count, '497');
    assert.equal(first.metadata.sense_count, '514');
    assert.equal(first.metadata.relation_count, '442');
    assert.equal(first.metadata.expression_count, '20');
