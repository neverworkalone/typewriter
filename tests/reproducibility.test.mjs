...
test('two independent builds have the same logical snapshot and provenance', async () => {
  ...
  try {
    ...
    assert.equal(first.metadata.record_count, '326');
    assert.equal(first.metadata.start_count, '300');
    assert.equal(first.metadata.reference_only_count, '26');
    assert.equal(first.metadata.candidate_count, '300');
    assert.equal(first.metadata.search_form_count, '363');
    assert.equal(first.metadata.sense_count, '386');
    assert.equal(first.metadata.relation_count, '340');
    assert.equal(first.metadata.expression_count, '14');
