...
export { M5_15_BASE_SUMMARY, M5_15_FINAL_SUMMARY, M5_15_TARGET };

export class M515ValidationError extends Error {
  constructor(message, code = 'M5_15_VALIDATION_ERROR') {
    super(message);
    this.name = 'M515ValidationError';
    this.code = code;
  }
}

function fail(message, code = 'M5_15_VALIDATION_ERROR') {
  throw new M515ValidationError(message, code);
}
...
function assertEqual(actual, expected, label, code = 'M5_15_VALIDATION_MISMATCH') {
...
export function validateM515Catalog(catalog = M5_15_CATALOG) {
  if (!Array.isArray(catalog) || catalog.length !== M5_15_SELECTION_COUNT) {
    fail(`M5-15 catalog must contain ${M5_15_SELECTION_COUNT} capacity slots`, 'CATALOG_SHAPE_ERROR');
  }
  const slots = new Set();
  for (const [index, entry] of catalog.entries()) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)
      ...
      || !/^m5-15-slot-[0-9]{4}$/u.test(entry.slot_id)
      || !Array.isArray(entry.flags) || entry.flags.length === 0
      || !['E', 'Q', 'S', 'C', 'A', 'O', 'X'].includes(entry.axis)) {
      fail(`M5-15 catalog row ${index} is invalid`, 'CATALOG_SHAPE_ERROR');
    }
    if (slots.has(entry.slot_id)) fail(`M5-15 catalog has duplicate ${entry.slot_id}`, 'CATALOG_SHAPE_ERROR');
...
function validateReviewArtifact(review, decisionSource, decisionSourceBytes) {
  if (!review || typeof review !== 'object') fail('M5-15 review artifact must be an object', 'REVIEW_SHAPE_ERROR');
  assertEqual(review.issue, 101, 'review issue');
  assertEqual(review.batch_id, M5_15_BATCH_ID, 'review batch ID');
  ...
  assertEqual(review.candidate_pool.selection_slot_count, M5_15_SELECTION_COUNT, 'selection capacity');
  assertEqual(review.candidate_pool.candidate_identity_count, M5_15_CANDIDATE_IDENTITIES.length, 'candidate identity count');
  assertEqual(review.candidate_pool.import_target, M5_15_IMPORT_COUNT, 'import target');
  assertEqual(review.candidate_pool.reserve_count, M5_15_RESERVE_COUNT, 'reserve capacity');
  assertEqual(review.candidate_pool.candidate_source_id, M5_15_CANDIDATE_SOURCE_ID, 'candidate source ID');
  ...
  assertEqual(review.decisions, {
    ...
    imported_start_count: M5_15_IMPORT_COUNT,
  }, 'review decisions');
  assertEqual(review.decision_artifact.source_id, M5_15_SEMANTIC_DECISION_SOURCE_ID, 'decision source ID');
...
function validateStageArtifact(stage, review, decisionSource, currentDirectory) {
  if (!stage || typeof stage !== 'object') fail('M5-15 stage artifact must be an object', 'STAGE_SHAPE_ERROR');
  assertEqual(stage.stage_id, 'm5-15-plus-1000', 'stage ID');
  assertEqual(stage.issue, 101, 'stage issue');
  ...
  assertEqual(stage.target, M5_15_TARGET, 'stage target');
  assertEqual(stage.actual.canonical_snapshot, M5_15_FINAL_SUMMARY, 'prospective canonical summary');
  assertEqual(stage.actual.imported_start_count, M5_15_IMPORT_COUNT, 'stage imported count');
  const disposition = selectionDispositionSummary(decisionSource.rows, { selection: decisionSource.selection });
  ...
  assertEqual(stage.source.candidate_source, 'data/batches/m5-15-lexical-unit-source.json', 'candidate source path');
  assertEqual(stage.source.candidate_source_sha256, sha256(M5_15_CANDIDATE_SOURCE_BYTES), 'candidate source bytes digest');
  assertEqual(stage.source.candidate_source_artifact_sha256, M5_15_CANDIDATE_SOURCE.artifact_sha256, 'candidate source artifact digest');
  assertEqual(stage.source.semantic_decision_source, path.relative(REPOSITORY_DIRECTORY, M5_15_SEMANTIC_DECISION_SOURCE_PATH), 'semantic decision source path');
  assertEqual(stage.source.semantic_decision_source_sha256, decisionSource.sourceSha256, 'semantic decision source digest');
  assertEqual(stage.predecessor_expansion.issue, 100, 'predecessor issue');
  assertEqual(stage.source.predecessor_stage, 'data/batches/m5-14-stage.json', 'predecessor stage path');
  assertEqual(stage.source.predecessor_stage_sha256, sha256(readFileSync(PREDECESSOR_STAGE_PATH)), 'predecessor stage digest');
  assertEqual(stage.previous_stage.checkpoint_commit, '810e7bdc6c19d7ceb0ab69d3767c4565e236de60', 'predecessor checkpoint commit');
  return true;
}
...
export async function validateM515({
  ...
  const sourceFile = await readM515DecisionSource();
  const candidateRecords = buildM515CandidateRecords();
  const sourceCandidates = candidateRecordsFromM515DecisionSource(sourceFile.source);
  assertEqual(candidateRecords, sourceCandidates, 'live candidate producer binding', 'CANDIDATE_SOURCE_MISMATCH');
  const decisionSource = validateM515DecisionSource({
    source: sourceFile.source,
    sourceBytes: sourceFile.sourceBytes,
    candidateRecords,
  });
  validateM515Catalog();
  const reviewFile = await readJson(reviewPath, 'M5-15 review');
  const review = reviewFile.value;
  validateReviewArtifact(review, decisionSource, sourceFile.sourceBytes);
  const stageFile = await readJson(stagePath, 'M5-15 stage');
