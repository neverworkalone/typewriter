...
// M5-13 is generated from a Typewriter-owned identity source.  Each identity
// binds a writer-facing root, a semantic focus, and the intended use boundary.
// The shared producer turns these source facts into candidate bodies; selection
// and admission are separate authored verification outcomes.
...
const SOURCE_GROUPS = Object.freeze({
  E: Object.freeze({
    ...
    focuses: Object.freeze([
      ['문턱', '시작 직전의 망설임', '막 열리려는 장면'],
      ['안쪽', '겉으로 보이지 않는 마음의 방향', '조용히 들여다보는 장면'],
      ['뒷면', '드러난 감정 뒤에 남은 다른 결', '말이 끝난 뒤의 장면'],
      ['여운', '사건이 지나간 뒤에도 남는 감각', '늦게 되돌아오는 장면'],
      ['윤곽', '흐릿한 의미가 형태를 얻는 과정', '빛과 그림자가 맞닿는 장면'],
      ['틈새', '감정과 감정 사이에 생긴 작은 간격', '잠시 비워진 장면'],
      ['파문', '한 감정이 주변으로 번져 가는 흔적', '주변의 표정이 달라지는 장면'],
      ['속살', '감정의 가장 가까운 중심에서 느껴지는 결', '마음의 표면이 걷힌 장면'],
    ]),
  }),
  Q: Object.freeze({
    record_type: 'entry',
    ...
    focuses: Object.freeze([
      ['단차', '같은 장면 안에서 느껴지는 미세한 높이 차이', '한 칸 낮은 바닥', '변화의 폭'],
      ['농도', '감각이 옅고 짙어지는 정도', '색이 번진 종이', '옅어지는 경계'],
      ['방향', '시선과 몸이 향하는 쪽', '돌아가는 길', '바뀌는 흐름'],
      ['간격', '두 요소 사이에 남은 거리', '나란한 의자', '벌어진 자리'],
      ['높낮이', '소리와 감정이 오르내리는 폭', '낭독의 한 구절', '달라진 음역'],
      ['속결', '표면 안쪽에서 느껴지는 질감의 방향', '접힌 천', '손끝에 남는 감각'],
      ['박자', '반복되는 움직임의 간격', '걸음을 맞춘 길', '이어지는 동작'],
      ['잔차', '변화 뒤에도 남아 있는 작은 차이', '지워진 문장', '남은 흔적'],
    ]),
  }),
  S: Object.freeze({
    record_type: 'entry',
    ...
    focuses: Object.freeze([
      ['잔향', '소리가 사라진 뒤 남는 울림', '문이 닫힌 뒤', '뒤늦은 소리'],
      ['기척', '보이지 않는 존재를 감지하는 작은 신호', '빈 방의 가장자리', '다가오는 흔적'],
      ['표면', '감각이 처음 닿는 바깥쪽의 자리', '손이 닿은 곳', '드러난 결'],
      ['깊이', '감각이 몸과 기억 안쪽으로 들어가는 정도', '오래 머문 장면', '안쪽의 울림'],
      ['온도', '감각이 따뜻하거나 차갑게 느껴지는 정도', '손바닥 가까이', '달라진 기운'],
      ['결', '감각이 이어지는 미세한 방향', '천을 문지른 자리', '손끝의 방향'],
      ['파장', '감각이 주변으로 넓어지는 흐름', '물결이 번진 곳', '퍼지는 울림'],
      ['잔상', '감각이 사라진 뒤 눈과 몸에 남는 이미지', '불이 꺼진 뒤', '늦게 남는 빛'],
    ]),
  }),
  C: Object.freeze({
    record_type: 'entry',
    ...
    focuses: Object.freeze([
      ['가장자리', '장면의 중심에서 조금 비켜난 경계', '시선이 닿지 않는 끝', '비켜 선 풍경'],
      ['기척', '사람이나 움직임이 남기는 작은 흔적', '문 뒤의 공간', '늦게 들린 소리'],
      ['풍경', '장소와 빛과 움직임이 한 번에 보이는 모습', '멀리 열린 시야', '한 덩어리의 장면'],
      ['시간', '장면에 쌓이거나 흐르는 변화의 감각', '오래된 벽면', '겹쳐진 하루'],
      ['문턱', '안과 밖이 만나는 이동의 경계', '발이 닿은 경계', '멈춘 이동'],
      ['빛결', '빛이 표면을 지나며 만드는 흐름', '기울어진 햇살', '흔들리는 밝기'],
      ['습기', '공기와 표면에 남아 있는 물기의 감각', '비가 그친 뒤', '젖은 냄새'],
      ['폐색', '장면의 출구와 시야가 막힌 상태', '끝나지 않는 벽', '닫힌 방향'],
    ]),
  }),
  A: Object.freeze({
    record_type: 'entry',
    ...
    focuses: Object.freeze([
      ['방향', '움직임이 향하는 쪽과 바뀌는 쪽', '갈림길의 순간', '달라진 진로'],
      ['리듬', '행동의 반복과 멈춤이 만드는 박자', '걸음이 이어지는 길', '바뀌는 속도'],
      ['순간', '행동이 시작되거나 끝나는 짧은 시간', '손이 닿기 직전', '짧은 변화'],
      ['간격', '행동 사이에 남는 시간과 거리', '말이 끊긴 자리', '비워 둔 움직임'],
      ['힘', '몸과 물건에 가해지는 움직임의 세기', '무거운 문 앞', '달라진 압력'],
      ['속도', '행동이 공간을 가르는 빠르기', '급해진 길', '짧아진 호흡'],
      ['자세', '몸이 행동 속에서 취하는 모양', '기울어진 의자', '남은 몸짓'],
      ['흔적', '행동이 지나간 뒤 표면에 남는 결과', '정리된 책상', '뒤늦은 변화'],
      ['끝맺음', '행동이 더 이어지지 않고 닫히는 지점', '문장이 멈춘 자리', '닫힌 움직임'],
    ]),
  }),
  O: Object.freeze({
    record_type: 'entry',
    ...
    focuses: Object.freeze([
      ['표면', '손과 빛이 처음 만나는 바깥쪽', '손이 닿은 자리', '드러난 결'],
      ['모서리', '면과 면이 만나 방향을 바꾸는 자리', '빛이 꺾인 자리', '달라진 윤곽'],
      ['무게', '손과 몸에 전달되는 물건의 압력', '들어 올린 순간', '처진 자세'],
      ['틈', '물건의 안과 밖 사이에 남은 작은 공간', '닫히지 않은 서랍', '새어 나온 빛'],
      ['온기', '물건에 머물다 손으로 옮겨 오는 따뜻함', '막 내려놓은 잔', '남은 열'],
      ['소리', '물건이 움직이거나 닿으며 내는 신호', '복도의 바닥', '짧은 울림'],
      ['자리', '물건이 놓여 공간에 남긴 위치', '비워진 책상', '남은 흔적'],
    ]),
  }),
  X: Object.freeze({
    ...
    focuses: Object.freeze([
      ['가라앉다', '높아진 감정이 낮은 곳으로 내려가는 변화', '소리가 줄어든 방', '긴장을 낮추는 문장'],
      ['머뭇거리다', '결정과 행동 사이에서 잠시 멈추는 변화', '문 앞의 짧은 시간', '망설임을 보여 주는 문장'],
      ['번져가다', '감정과 장면이 주변으로 넓어지는 변화', '빛이 퍼진 벽', '영향의 범위를 넓히는 문장'],
      ['돌아오다', '떠난 대상과 마음이 다시 현재로 향하는 변화', '익숙한 골목', '회귀의 느낌을 남기는 문장'],
      ['불안해지다', '한 방향에 머물던 상태가 안정성을 잃고 걱정으로 기우는 변화', '바람 부는 창', '불안을 보여 주는 문장'],
      ['멈춰서다', '움직임이 한순간 중단되어 장면을 바꾸는 변화', '건널목 앞', '정지를 강조하는 문장'],
      ['스며들다', '감정과 감각이 안쪽으로 천천히 들어가는 변화', '젖은 천', '잔잔한 변화를 남기는 문장'],
    ]),
  }),
});
...
export const M5_13_VERIFICATION_PASS_ID = 'm5-13-agent-semantic-review-20260922-r2';
export const M5_13_CANDIDATE_SOURCE_ID = 'm5-13-typewriter-authored-identities-20260922-r2';
export const M5_13_GENERATOR_VERSION = 'm5-13-shared-lexical-producer-v2';
export const M5_13_SEMANTIC_REVIEW_VERSION = 'm5-13-authored-semantic-review-v2';
...
function makeIdentity(catalogRow, group, root, focus, rootIndex, focusIndex) {
  ...
  const sourceBasis = {
    ...
    focus_scene: focus[2],
    focus_cue: focus[3] ?? focus[1],
    composition_pattern: isExpression ? 'root-focus-expression' : 'root-reading-focus',
    source_material: 'Typewriter-authored writer-facing root/focus identity',
    source_position: { root_index: rootIndex, focus_index: focusIndex },
  };
  return {
