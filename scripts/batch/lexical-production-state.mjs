...
function parseStageSource(stageId, sourceBytes, label = `production stage ${stageId}`) {
  ...
  return { bytes, sourceSha256, payloadSha256 };
}
...
function makeStage(run, stageId, {
  ...
  const parsedSource = sourceInputForStage(stageId, sourceBytes, sourcePath);
...
export function produceLexicalProductionState({
  ...
  const run = createLexicalProductionRun({ batchId, pipelineVersion });
  ...
  for (const stageId of LEXICAL_PRODUCTION_STAGE_IDS.slice(0, -1)) {
    ...
    const stageBytes = productionStageBytes(stageId, asBytes(input.source_bytes, `production stage evidence.${stageId}.source_bytes`));
    const methodName = {
  ...
  const admissionStageBytes = productionStageBytes(
    'admission',
    asBytes(admissionInput.source_bytes, 'production stage evidence.admission.source_bytes'),
  );
  ...
  const sourceBytesByStage = Object.fromEntries(LEXICAL_PRODUCTION_STAGE_IDS.map((stageId) => {
    const input = stageValues[stageId];
    return [stageId, productionStageBytes(
      stageId,
      asBytes(input.source_bytes, `production stage evidence.${stageId}.source_bytes`),
    )];
  }));
  return { state, sources: sourceBytesByStage };
}
...
function normalizeValidationSourceBytes(stageId, sourceBytes) {
  ...
  return productionStageBytes(stageId, bytes);
}

function validateStage(stage, stageId, sourceBytesByStage, state, seenSources, seenPayloads) {
