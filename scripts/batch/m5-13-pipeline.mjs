...
export const M5_13_BASE_LEDGER_SHA256 =
  '35b0ee53ba199d1ecbd14cbca160db1928ece6989f70642572b35478ca94a39a';
...
function buildGate({ identities, reviewRows, production, semanticAuditCoverage, finalSummary, relation, preflight, candidateSourceDigest, generationPassId, verificationPassId }) {
  ...
  const qualityPasses = {
    ...
    source_quality_selection: decisions.deferred === M5_13_RESERVE_COUNT && decisions.held === 0 && decisions.rejected === 0,
...
function buildAdmission({ inputs, artifacts, prospective, semanticAudit, semanticAuditCoverage, preflight, gate, decisionSourceBytes, promotionLedgerBytes, promotionLedgerBinding, baseDecisionSource, production }) {
  ...
  return {
    ...
    base: {
      ...
      inventory_sha256: M5_13_BASE_INVENTORY_SHA256,
      seed_sha256: M5_13_BASE_SEED_SHA256,
...
async function buildBaseInventory({ baseSeed, basePromotionLedger }) {
  const artifact = await readJson(BASE_INVENTORY_PATH, 'M5-13 base inventory');
  if (sha256(artifact.bytes) !== M5_13_BASE_INVENTORY_SHA256) {
    fail('M5-13 base inventory digest drifted', 'BASE_INVENTORY_MISMATCH');
  }
  return { inventory: artifact.value, bytes: artifact.bytes };
}

async function loadBaseInputs({ currentSeedPath = CURRENT_SEED_PATH, currentPromotionLedgerPath = CURRENT_PROMOTION_LEDGER_PATH, decisionSourcePath = DECISION_SOURCE_PATH } = {}) {
  ...
  if (baseCanonicalDigest !== M5_13_BASE_CANONICAL_SHA256) fail('M5-13 base canonical digest drifted', 'BASE_CANONICAL_MISMATCH');
  ...
  if (sha256(baseSeedBytes) !== M5_13_BASE_SEED_SHA256) fail('M5-13 base seed digest drifted', 'BASE_SEED_MISMATCH');
  ...
  if (sha256(serializePromotionLedger(basePromotionLedger)) !== M5_13_BASE_LEDGER_SHA256) fail('M5-13 base promotion ledger digest drifted', 'PROMOTION_LEDGER_HISTORY_MISMATCH');
  ...
  const baseInventory = await buildBaseInventory({ baseSeed, basePromotionLedger });
  return {
    baseCanonical,
    baseRecords,
    baseCanonicalDigest,
...
export async function buildM513({
  ...
  const inputs = await loadBaseInputs({ currentSeedPath, currentPromotionLedgerPath, decisionSourcePath });
  ...
  const generatedSource = buildM513DecisionSource({ identities, candidateRecords: generatedCandidates });
  if (sha256(sourceFile.sourceBytes) !== sha256(generatedSource.bytes)) {
    fail('M5-13 durable semantic decision source is not the deterministic shared-producer output', 'M5_13_DECISION_SOURCE_MISMATCH');
  }
  const candidateRecords = candidateRecordsFromM513DecisionSource(sourceFile.source, identities);
  ...
  if (JSON.stringify(canonicalSummary(prospective.canonical.records)) !== JSON.stringify(M5_13_FINAL_SUMMARY)) fail('M5-13 prospective canonical summary is not exactly +1,000', 'FINAL_COUNT_MISMATCH');
...
export async function commitM513PromotionTransaction({ result, currentCanonicalDirectory = CURRENT_CANONICAL_DIRECTORY, currentSeedPath = CURRENT_SEED_PATH, promotionLedgerPath = CURRENT_PROMOTION_LEDGER_PATH, decisionSourcePath = DECISION_SOURCE_PATH, admissionPath = ADMISSION_PATH, promotionPath = PROMOTION_PATH } = {}) {
  ...
  if (currentCanonicalDigest !== result.inputs.baseCanonicalDigest
    || sha256(currentSeedBytes) !== M5_13_BASE_SEED_SHA256
    || sha256(currentLedgerBytes) !== M5_13_BASE_LEDGER_SHA256
...
export async function validateM513Final({ currentCanonicalDirectory = CURRENT_CANONICAL_DIRECTORY, currentSeedPath = CURRENT_SEED_PATH, promotionLedgerPath = CURRENT_PROMOTION_LEDGER_PATH, decisionSourcePath = DECISION_SOURCE_PATH, admissionPath = ADMISSION_PATH, promotionPath = PROMOTION_PATH } = {}) {
  const promotion = (await readJson(promotionPath, 'M5-13 promotion evidence')).value;
  const result = await buildM513({ currentSeedPath, currentPromotionLedgerPath: promotionLedgerPath, decisionSourcePath, preflightRunner: async () => promotion.preflight });
  ...
  validatePromotionLedgerBindings({ entries: currentLedgerEntries, canonicalRecords: currentCanonical.records, decisionSource });
  ...
  if (currentDigest !== result.prospective.canonicalDigest || sha256(currentSeedBytes) !== sha256(result.prospective.seedBytes) || sha256(currentDecisionBytes) !== sha256(result.decisionSourceBytes)) fail('M5-13 final output digest drifted', 'FINAL_DIGEST_MISMATCH');
...
async function generateDecisionSource() {
  const generated = buildM513DecisionSource();
  await writeFile(M5_13_SEMANTIC_DECISION_SOURCE_PATH, generated.bytes);
  return { path: sourcePath(M5_13_SEMANTIC_DECISION_SOURCE_PATH), sha256: sha256(generated.bytes), bytes: generated.bytes.length };
}

export async function main(argv = process.argv.slice(2)) {
  if (argv.includes('--generate-source')) {
    console.log(JSON.stringify(await generateDecisionSource(), null, 2));
    return;
  }
