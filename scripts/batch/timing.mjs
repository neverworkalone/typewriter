...
export const CALIBRATION_AUDITED_CASE_IDS = Object.freeze(
  Array.from({ length: 20 }, (_, index) => `m5-10a-cal-${String(index + 1).padStart(3, '0')}`),
);
export const CALIBRATION_TIMING_RECORDING_COMMAND = 'node scripts/batch/record-m5-10a-calibration-timing.mjs';
export const CALIBRATION_MIN_FINAL_AUDIT_SECONDS_PER_CASE = 1;
...
function normalizeFinalAuditWorkEvidence(workEvidence) {
  if (!workEvidence || typeof workEvidence !== 'object') {
    fail('final-audit must include audited case coverage', 'TIMING_AUDIT_COVERAGE_REQUIRED');
  }
  const caseIds = workEvidence.case_ids;
  if (!Array.isArray(caseIds)
    || caseIds.length !== CALIBRATION_AUDITED_CASE_IDS.length
    || caseIds.some((caseId) => (
      typeof caseId !== 'string' || !/^m5-10a-cal-[0-9]{3}$/u.test(caseId)
    ))
    || new Set(caseIds).size !== caseIds.length) {
    fail('final-audit work evidence must list each audited calibration case once', 'TIMING_AUDIT_COVERAGE_REQUIRED');
  }
  if (workEvidence.case_count !== CALIBRATION_AUDITED_CASE_IDS.length
    || CALIBRATION_AUDITED_CASE_IDS.some((caseId) => !caseIds.includes(caseId))) {
    fail('final-audit work evidence case_count must match case_ids', 'TIMING_AUDIT_COVERAGE_REQUIRED');
  }
  if (typeof workEvidence.raw_proposal_sha256 !== 'string'
    || !/^[a-f0-9]{64}$/u.test(workEvidence.raw_proposal_sha256)) {
    fail('final-audit work evidence must include a raw-proposal SHA-256 digest', 'TIMING_AUDIT_DIGEST_REQUIRED');
  }
  return {
    case_ids: [...caseIds],
    case_count: workEvidence.case_count,
    raw_proposal_sha256: workEvidence.raw_proposal_sha256,
  };
}
...
export function stopCalibrationTimingPass(pass, { now, workEvidence } = {}) {
  ...
  const normalizedWorkEvidence = pass.id === 'final-audit'
    ? normalizeFinalAuditWorkEvidence(workEvidence)
    : undefined;
  if (pass.id !== 'final-audit' && workEvidence !== undefined) {
    fail(`work evidence is only valid for final-audit, not ${pass.id}`, 'TIMING_AUDIT_COVERAGE_UNEXPECTED');
  }
  ...
  if (pass.id === 'final-audit') {
    const minimumSeconds = normalizedWorkEvidence.case_count
      * CALIBRATION_MIN_FINAL_AUDIT_SECONDS_PER_CASE;
    if (elapsedSeconds < minimumSeconds) {
      fail(
        `final-audit recorded ${elapsedSeconds} editor seconds for ${normalizedWorkEvidence.case_count} audited cases; at least ${minimumSeconds} seconds are required`,
  ...
  return {
    ...
    ...(normalizedWorkEvidence ? { work_evidence: normalizedWorkEvidence } : {}),
  };
}
...
export function verifyCalibrationTimingRecording(recording) {
  ...
  for (const pass of recording.passes) {
    ...
    if (pass.id === 'final-audit') {
      finalAuditWorkEvidence = normalizeFinalAuditWorkEvidence(pass.work_evidence);
      const minimumSeconds = finalAuditWorkEvidence.case_count
        * CALIBRATION_MIN_FINAL_AUDIT_SECONDS_PER_CASE;
      if (pass.editor_seconds < minimumSeconds) {
        fail(
          `final-audit recorded ${pass.editor_seconds} editor seconds for ${finalAuditWorkEvidence.case_count} audited cases; at least ${minimumSeconds} seconds are required`,
          'TIMING_AUDIT_DURATION_TOO_SHORT',
        );
      }
    } else if (Object.hasOwn(pass, 'work_evidence')) {
      fail(`${pass.id} contains unexpected audit work evidence`, 'TIMING_AUDIT_COVERAGE_UNEXPECTED');
    }
  ...
  return {
    ...
    final_audit_pass_session_id: recording.passes.find(({ id }) => id === 'final-audit').session_id,
    final_audit_work_evidence: finalAuditWorkEvidence,
  };
}
