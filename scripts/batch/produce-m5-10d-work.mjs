/**
 * Proposal-only producer for M5-10D.
 *
 * The producer is deliberately unaware of editorial outcomes. It accepts a
 * compact, source-bound workload input and returns provenance only. Decisions,
 * corrections, relation admission, and audit findings are authored and
 * validated in separate artifacts.
 */

import { createHash } from 'node:crypto';

export const M5_10D_PRODUCER_VERSION = 'm5-10d-proposal-producer-v1';
...
const RELATION_TYPES = Object.freeze(['direct', 'near', 'mood', 'scene', 'sensory', 'action', 'association', 'antonym']);
...
function assertSenseIds(input) {
  if (!Array.isArray(input.source_sense_ids) || input.source_sense_ids.length === 0) {
  ...
  if (new Set(input.source_sense_ids).size !== input.source_sense_ids.length) {
    fail('proposal input contains duplicate source sense IDs');
  }
}
...
function assertCandidate(candidate, sourceSenseIds) {
  ...
  if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) {
    fail('relation_candidate must be an object or null');
  }
  ...
  if (!sourceSenseIds.includes(candidate.source_sense)) {
    fail('relation candidate source sense is not in the proposal record');
  }
  ...
  if (!RELATION_TYPES.includes(candidate.type)) fail('relation candidate type is invalid');
  if (!candidate.direction || candidate.direction.from !== candidate.source_sense || candidate.direction.to !== candidate.target_sense) {
...
export function proposalInputFromCase(caseRecord) {
  ...
  const compact = {
    ...
    source_sense_ids: record.senses.map(({ id }) => id),
...
export function produce({ unitId, unitKind, input } = {}) {
  ...
  if (input.phase !== 'proposal') fail('M5-10D producer accepts proposal facts only');
  ...
  if (!SHA256_PATTERN.test(requireString(input.source_record_sha256, 'proposal input.source_record_sha256'))) {
    fail('proposal input.source_record_sha256 is invalid');
  }
  assertSenseIds(input);
  if (!Array.isArray(input.source_pos) || input.source_pos.length !== input.source_sense_ids.length) {
    fail('proposal input.source_pos must align with source_sense_ids');
  }
  ...
  if (unitId !== input.case_id) fail('producer unitId must match proposal input.case_id');
  return {
    producer_version: M5_10D_PRODUCER_VERSION,
    phase: 'proposal',
    unit_id: unitId,
    unit_kind: unitKind,
