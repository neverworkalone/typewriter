...
export const M5_13_REVIEW_SHA256 =
  '18e4e2dea42c9f2cda169d0f5ef051cd6bf4e469cb36b74c4fd40af67b9822eb';
...
export async function validateM513({
  ...
  assertEqual(stage.status, 'blocked-upstream-source', 'stage status');
  ...
  assertEqual(review.status, 'blocked-upstream-source', 'review status');
  ...
  assertEqual(review.decision_artifact, null, 'decision artifact must be absent until the source contract exists');
  assertEqual(
    review.source_policy.candidate_bodies,
    'blocked-missing-source-bound-candidate-contract',
    'candidate source contract status',
  );
  assertEqual(
    review.source_policy.raw_external_material,
    'not-adopted-pending-source-review',
    'external source policy status',
  );
  assertEqual(review.audit.status, 'blocked-upstream-source', 'audit status');
  ...
  assertEqual(stage.gate.failures, [
    ...
    'canonical_promotion',
  ], 'upstream capability gate failures');
